...
public class MainMenu : MonoBehaviour // stores UI functions
{
    ...
    {
        if (BossProgress.Instance.IsBossDefeated("Gluttony") && !BossProgress.Instance.IsBossDefeated("Wrath")) // if first boss defeated
        {
            UnityEngine.SceneManagement.SceneManager.LoadScene("WrathBoss"); // load second
        }
        else if (BossProgress.Instance.IsBossDefeated("Wrath") && !BossProgress.Instance.IsBossDefeated("Envy")) // if second boss defeated
        {
